...
impl CATapDescription {
  pub fn init_stereo_mixdown_of_processes(
    ...
    let cls =
      AnyClass::get("CATapDescription").ok_or(CoreAudioError::CATapDescriptionClassNotFound)?;
    let obj: *mut AnyObject = unsafe { msg_send![cls, alloc] };
    if obj.is_null() {
      return Err(CoreAudioError::AllocCATapDescriptionFailed);
    }
    let processes_array =
      NSArray::from_id_slice(&[NSNumber::initWithUnsignedInt(NSNumber::alloc(), process)]);
    let obj: *mut AnyObject =
      unsafe { msg_send![obj, initStereoMixdownOfProcesses: processes_array.as_ref()] };
    if obj.is_null() {
  ...
  pub fn init_stereo_global_tap_but_exclude_processes(
    ...
    let processes_array = NSArray::from_id_slice(
      &processes
        .iter()
